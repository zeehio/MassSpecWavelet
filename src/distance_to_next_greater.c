...
static void distance_to_next_greater_i(int *x, R_xlen_t xlength, int *outi) {
    for (R_xlen_t i = 0; i < xlength; i++) {
        outi[i] = R_NaInt;
    }
    int st = 0;
    for (R_xlen_t i = 0; i < xlength - 1; i++) {
        if (x[i] < x[i+1]) {
            outi[i] = 1;
        } else {
            st = i + 1;
...
static void distance_to_next_greater_d(double *x, R_xlen_t xlength, int *outi) {
    for (R_xlen_t i = 0; i < xlength; i++) {
        outi[i] = R_NaInt;
    }
    int st = 0;
...
SEXP distance_to_next_greater(SEXP s_x) {
    double *xd;
    int *xi;
    ...
    if (is_int) {
        distance_to_next_greater_i(xi, xlength, outi);
    } else {
        distance_to_next_greater_d(xd, xlength, outi);
    }
    Rf_unprotect(1);
    return out;
}
