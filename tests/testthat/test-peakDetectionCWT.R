test_that("a single skinny peak is detected at the correct index regardless of padding", {
    skinny_peak <- c(
    ...
    expect_length(peak2, 1)
    expect_equal(unname(peak2), unname(peak1) + 256)
})

test_that("a wider peak is detected at the correct index regardless of padding", {
    ...
    expect_equal(unname(peak1$majorPeakInfo$peakIndex), 30)
    ...
    expect_equal(unname(peak2$majorPeakInfo$peakIndex), 30 + 256)
})

test_that("peakDetectionCWT() does not crash on a flat/constant signal", {
