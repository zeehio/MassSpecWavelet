...
- We compare the required CPU time of both

Finally, we show how this new `localMaxima()` algorithm affects the detection of
local maxima in wavelet coefficients at the `exampleMS` data.

For now, the peak detection pipeline still uses the `"classic"` algorithm, since
we want to explore in detail how the changes in the local maxima detection
impact the whole peak detection pipeline. If you want to test using this new
local maxima algorithm within the peak detection pipeline, you can set
...
for (winSize in winSizes) {
    for (xlength in xlengths) {
        ...
        out[[i]] <- data.frame(
            ...
            min_cpu_time_ms = as.numeric(bm$min),
            xlength = xlength,
            winSize = winSize
        )
    }
...
plot(
    ...
    col = ifelse(out2$algorithm == "new", "red", "blue"),
...
legend(
    "topleft",
    legend = c("New algorithm", "Classic algorithm",
